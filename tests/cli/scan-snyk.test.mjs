...
import { parseSnykFindings, attachVerdicts } from '../../.claude/scan-snyk.mjs'

const sarif = {
  runs: [{
    results: [
      { level: 'error', locations: [{ physicalLocation: { artifactLocation: { uri: 'a.mjs' } } }] },
      { level: 'warning', locations: [{ physicalLocation: { artifactLocation: { uri: 'a.mjs' } } }] },
      { level: 'note', locations: [{ physicalLocation: { artifactLocation: { uri: 'b.mjs' } } }] },
      { level: 'error', locations: [{ physicalLocation: { artifactLocation: { uri: 'dir/b.mjs' } } }] },
    ],
  }],
}
...
describe('parseSnykFindings', () => {
  ...
  it('niveau inconnu → low', () => {
    const c = parseSnykFindings({ runs: [{ results: [{ level: 'mystery', locations: [{ physicalLocation: { artifactLocation: { uri: 'x.mjs' } } }] }] }] })
...
describe('attachVerdicts', () => {
  it('attache un verdict à chaque hook avec code_snippet', () => {
    ...
    const updated = attachVerdicts(hooks, { 'a.mjs': { high: 2, medium: 0, low: 0 } }, '2026-06-03T00:00:00Z')
    expect(updated).toEqual(['a', 'b'])
    expect(hooks[0].implementation.security.snyk).toEqual({ high: 2, medium: 0, low: 0, scannedAt: '2026-06-03T00:00:00Z' })
  })
  it('hook propre (sans finding) → 0/0/0', () => {
    const hooks = [{ slug: 'b', implementation: { code_snippet: 'y' } }]
    attachVerdicts(hooks, {}, 'T')
    expect(hooks[0].implementation.security.snyk).toEqual({ high: 0, medium: 0, low: 0, scannedAt: 'T' })
  })
