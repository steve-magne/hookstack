...
describe('aggregateAlerts', () => {
  ...
  it('counts by security_severity_level', () => {
    const alerts = [
      { rule: { security_severity_level: 'high' }, most_recent_instance: { location: { path: '.claude/hooks/foo.mjs' } } },
      { rule: { security_severity_level: 'medium' }, most_recent_instance: { location: { path: '.claude/hooks/foo.mjs' } } },
      { rule: { security_severity_level: 'critical' }, most_recent_instance: { location: { path: '.claude/hooks/bar.mjs' } } },
    ]
    const result = aggregateAlerts(alerts)
    expect(result['foo.mjs']).toEqual({ critical: 0, high: 1, medium: 1, low: 0 })
    expect(result['bar.mjs']).toEqual({ critical: 1, high: 0, medium: 0, low: 0 })
  })

  it('falls back to rule.severity when security_severity_level is absent', () => {
    const alerts = [
      { rule: { severity: 'error' }, most_recent_instance: { location: { path: '.claude/hooks/foo.mjs' } } },
      { rule: { severity: 'warning' }, most_recent_instance: { location: { path: '.claude/hooks/foo.mjs' } } },
    ]
    const result = aggregateAlerts(alerts)
    expect(result['foo.mjs']).toEqual({ critical: 0, high: 1, medium: 1, low: 0 })
  })
  ...
  it('skips alerts without a path', () => {
    const alerts = [{ rule: { security_severity_level: 'high' }, most_recent_instance: {} }]
...
describe('attachVerdicts', () => {
  ...
  it('writes found counts for hooks with alerts', () => {
    ...
    expect(hooks[0].implementation.security.codeql).toMatchObject({ high: 2, medium: 1 })
  })
