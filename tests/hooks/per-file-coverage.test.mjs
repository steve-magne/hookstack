...
describe("per-file-coverage", () => {
	...
	it("ignore src/types imbriqué dans un monorepo", () => {
		const exec = (cmd) =>
			cmd.includes("diff") ? "apps/web/src/types/hook.ts" : "";
		const result = run(makeOpts({ exec, readFile: () => "{}" }));
		expect(result.exitCode).toBe(0);
	});
	...
	it("ignore les fichiers absents du rapport de coverage", () => {
		const exec = (cmd) =>
			cmd.includes("diff") ? "src/lib/untracked.ts" : "abc";
		const result = run(makeOpts({ exec, readFile: () => "{}" }));
		expect(result.exitCode).toBe(0);
	});
});
