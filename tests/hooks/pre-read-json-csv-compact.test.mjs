...
const bigCsv = (n) =>
	[
		"id,name,email",
		...Array.from({ length: n }, (_, i) => `${i},user_${i},u${i}@x.com`),
	].join("\n");
const bigJsonl = (n) =>
	Array.from({ length: n }, (_, i) =>
		JSON.stringify({ id: i, val: i * 2 }),
	).join("\n");
...
describe("pre-read-json-csv-compact", () => {
	it("laisse passer si tool_name != Read", () => {
		expect(
			run({ tool_name: "Write", tool_input: { file_path: "data.json" } }),
		).toBeNull();
	});

	it("laisse passer si extension non supportée", () => {
		expect(
			run({ tool_name: "Read", tool_input: { file_path: "file.ts" } }),
		).toBeNull();
	});
	...
	it("laisse passer si le fichier n'existe pas", () => {
		const deps = {
			readFile: vi.fn(),
			exists: vi.fn(() => false),
			stat: vi.fn(() => ({ size: 100_000 })),
		};
		expect(
			run({ tool_name: "Read", tool_input: { file_path: "big.json" } }, deps),
		).toBeNull();
	});
	...
	it("laisse passer si fichier petit (< seuil)", () => {
		...
		expect(
			run({ tool_name: "Read", tool_input: { file_path: "small.json" } }, deps),
		).toBeNull();
	});
	...
	it("résume un grand tableau JSON", () => {
		...
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/users.json" } },
			deps,
		);
	...
	it("résume un grand objet JSON", () => {
		...
		const deps = makeDeps(content);
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/config.json" } },
			deps,
		);
		expect(result?.decision).toBe("block");
	...
	it("résume un grand CSV", () => {
		...
		const deps = makeDeps(content);
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/data.csv" } },
			deps,
		);
		expect(result?.decision).toBe("block");
	...
	it("résume un grand JSONL", () => {
		...
		const deps = makeDeps(content);
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/events.jsonl" } },
			deps,
		);
		expect(result?.decision).toBe("block");
	...
	it("laisse passer si JSON invalide", () => {
		...
		expect(
			run(
				{ tool_name: "Read", tool_input: { file_path: "/tmp/bad.json" } },
				deps,
			),
		).toBeNull();
	});
