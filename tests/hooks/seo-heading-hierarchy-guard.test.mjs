...
describe('seo-heading-hierarchy-guard', () => {
  it('ignore les fichiers hors src/', () => {
    expect(run(input('/p/docs/x.tsx'), { readFile: () => '<h1/><h1/>' })).toBeNull();
  });
  ...
  it('silencieux avec un seul <h1>', () => {
    expect(run(input(FILE), { readFile: () => '<h1>Title</h1><h2>Sub</h2><h2>Sub</h2>' })).toBeNull();
  });

  it('silencieux sans aucun <h1>', () => {
    expect(run(input(FILE), { readFile: () => '<h2>A</h2><h3>B</h3>' })).toBeNull();
  });
