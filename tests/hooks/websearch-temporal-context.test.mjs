...
describe('websearch-temporal-context', () => {
  it('ajoute l\'année si aucun contexte temporel', () => {
    const result = run(input('best React state management'), { currentYear: 2026 });
    expect(result?.hookSpecificOutput?.modifiedToolInput?.query).toBe('best React state management 2026');
  });

  it('ne modifie pas si une année est déjà présente', () => {
    expect(run(input('Next.js 15 features 2025'), { currentYear: 2026 })).toBeNull();
  });

  it('ne modifie pas si "latest" est présent', () => {
    expect(run(input('latest Claude models'), { currentYear: 2026 })).toBeNull();
  });

  it('ne modifie pas si "recent" est présent', () => {
    expect(run(input('recent AI breakthroughs'), { currentYear: 2026 })).toBeNull();
  });
