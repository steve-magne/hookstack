...
describe("seo-next-image-guard", () => {
	it("ignore les fichiers hors src/", () => {
		expect(
			run(input("/p/scripts/x.tsx"), { readFile: () => '<img src="a" />' }),
		).toBeNull();
	});

	it("ignore les fichiers non .tsx", () => {
		expect(
			run(input("/p/src/lib/x.ts"), { readFile: () => '<img src="a" />' }),
		).toBeNull();
	});
	...
	it("silencieux avec next/image <Image>", () => {
		expect(
			run(input(FILE), {
				readFile: () => '<Image src={x} alt="a" width={10} height={10} />',
			}),
		).toBeNull();
	});
	...
	it("signale un <img> brut", () => {
		const r = run(input(FILE), {
			readFile: () => 'return <img src="/a.png" alt="a" />;',
		});
	...
	it("ne confond pas <Image> avec <img>", () => {
		expect(
			run(input(FILE), { readFile: () => '<Image alt="x" />' }),
		).toBeNull();
	});
});
