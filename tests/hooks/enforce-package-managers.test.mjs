...
describe('enforce-package-managers', () => {
  ...
  it('laisse passer npm dans un message de commit git (-m "...")', () => {
    expect(run({ tool_name: 'Bash', tool_input: { command: 'git commit -m "support npm and yarn"' } })).toBeNull();
  });

  it('laisse passer yarn dans le body d\'une PR gh ("...")', () => {
    expect(run({ tool_name: 'Bash', tool_input: { command: 'gh pr create --body "yarn is now supported"' } })).toBeNull();
  });

  it('laisse passer npm dans un argument entre guillemets simples', () => {
    expect(run({ tool_name: 'Bash', tool_input: { command: "echo 'use npm here'" } })).toBeNull();
  });

  it('bloque npm après && même si la commande débute par git', () => {
    const r = run({ tool_name: 'Bash', tool_input: { command: 'git status && npm install' } });
    expect(r?.decision).toBe('block');
  });

  it('bloque yarn après && même si la commande débute par gh', () => {
    const r = run({ tool_name: 'Bash', tool_input: { command: 'gh auth status && yarn add lodash' } });
    expect(r?.decision).toBe('block');
  });
});
