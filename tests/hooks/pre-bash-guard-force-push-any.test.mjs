...
describe("guard-force-push-any", () => {
	it("bloque git push --force sur une branche feature", () => {
		expect(run(bash("git push --force origin feature/x"))?.decision).toBe(
			"block",
		);
	});
	...
	it("laisse passer --force-with-lease", () => {
		expect(
			run(bash("git push --force-with-lease origin feature/x")),
		).toBeNull();
	});
	...
	it("ignore --force dans une chaîne entre guillemets", () => {
		expect(
			run(bash('git commit -m "use git push --force carefully"')),
		).toBeNull();
	});

	it("ignore les outils non-Bash", () => {
		expect(
			run({ tool_name: "Edit", tool_input: { command: "git push -f" } }),
		).toBeNull();
	});
});
