...
describe("worktree-remove-cleanup", () => {
	it("retourne null sans worktree_path", () => {
		expect(
			run({}, { exec: vi.fn(), exists: () => true, rm: vi.fn() }),
		).toBeNull();
	});
	...
	it("lance docker compose down si compose présent", () => {
		...
		const r = run(
			{ worktree_path: "/wt" },
			{ exec, exists: (p) => p.includes("docker-compose.yml"), rm: vi.fn() },
		);
	...
	it("supprime node_modules si présent", () => {
		...
		const r = run(
			{ worktree_path: "/wt" },
			{ exec: vi.fn(), exists: (p) => p.endsWith("node_modules"), rm },
		);
		expect(rm).toHaveBeenCalledWith("/wt/node_modules", {
			recursive: true,
			force: true,
		});
	...
	it("ne fait rien si rien à nettoyer", () => {
		const r = run(
			{ worktree_path: "/wt" },
			{ exec: vi.fn(), exists: () => false, rm: vi.fn() },
		);
