...
describe('post-edit-conflict-marker-check', () => {
  ...
  it("laisse passer si le fichier n'existe pas", () => {
    expect(run({ tool_input: { file_path: 'gone.ts' } }, makeFsDeps('', false))).toBeNull();
  });

  it('laisse passer un fichier sain', () => {
    expect(run({ tool_input: { file_path: 'a.ts' } }, makeFsDeps('const x = 1;\n'))).toBeNull();
  });

  it('laisse passer un souligné markdown (======= seul)', () => {
    expect(run({ tool_input: { file_path: 'doc.md' } }, makeFsDeps(`Titre\n${SEP}\ntexte\n`))).toBeNull();
  });

  it("laisse passer une borne ouvrante sans fermante (faux positif improbable)", () => {
    expect(run({ tool_input: { file_path: 'a.ts' } }, makeFsDeps(`${OPEN}\ncode\n`))).toBeNull();
  });

  it('signale un fichier avec les deux bornes de conflit', () => {
    const r = run({ tool_input: { file_path: 'src/a.ts' } }, makeFsDeps(conflicted));
