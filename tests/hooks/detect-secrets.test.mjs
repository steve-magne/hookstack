...
describe("detect-secrets", () => {
	it("bloque une clé API Anthropic", () => {
		expect(
			run(cmd(["export ANTHROPIC_API_KEY=", SECRETS.anthropicKey]))?.decision,
		).toBe("block");
	});
	...
	it("bloque une clé privée", () => {
		expect(run(cmd(['echo "', SECRETS.privateKey, '"']))?.decision).toBe(
			"block",
		);
	});

	it("bloque un password=...", () => {
		expect(run(cmd(['curl -d "', SECRETS.passwordLine, '"']))?.decision).toBe(
			"block",
		);
	});
