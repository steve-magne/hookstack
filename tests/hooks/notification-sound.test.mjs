...
import {
	resolveActivateBundle,
	run,
} from "../../.claude/hooks/notification-sound.mjs";
...
describe("resolveActivateBundle", () => {
	...
	it("retourne Claude app si TERM_PROGRAM est absent (app desktop)", () => {
		expect(resolveActivateBundle(undefined)).toBe(
			"com.anthropic.claudefordesktop",
		);
	});

	it("retourne Claude app si TERM_PROGRAM est inconnu", () => {
		expect(resolveActivateBundle("some-unknown-terminal")).toBe(
			"com.anthropic.claudefordesktop",
		);
	});
});
...
describe("notification-sound", () => {
	it("utilise terminal-notifier avec le bon bundleId si disponible (iTerm2)", () => {
		...
		expect(exec).toHaveBeenCalledWith(
			expect.stringContaining("com.googlecode.iterm2"),
		);
	});
	...
	it("utilise terminal-notifier avec Claude app si pas de TERM_PROGRAM", () => {
		...
		expect(exec).toHaveBeenCalledWith(
			expect.stringContaining("com.anthropic.claudefordesktop"),
		);
	});
	...
	it("retourne null (non bloquant)", () => {
		const exec = vi.fn();
		expect(
			run({}, { exec, hasTerminalNotifier: () => true, platform: "darwin" }),
		).toBeNull();
	});
