...
describe("file-to-markdown", () => {
	...
	it("laisse passer si tool_name != Read", () => {
		expect(
			run(
				{ tool_name: "Write", tool_input: { file_path: "doc.pdf" } },
				{ exec: mockExec, exists: mockExists },
			),
		).toBeNull();
	});
	...
	it("laisse passer si tool_input absent", () => {
		expect(
			run({ tool_name: "Read" }, { exec: mockExec, exists: mockExists }),
		).toBeNull();
	});
	...
	it("laisse passer si le fichier n'existe pas", () => {
		...
		expect(
			run(
				{ tool_name: "Read", tool_input: { file_path: "doc.pdf" } },
				{ exec: mockExec, exists: mockExists },
			),
		).toBeNull();
	});
	...
	it("convertit un DOCX avec pandoc", () => {
		...
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/rapport.docx" } },
			{ exec: mockExec, exists: mockExists },
		);
		expect(result?.decision).toBe("block");
	...
	it("convertit un PPTX avec pandoc", () => {
		...
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/slides.pptx" } },
			{ exec: mockExec, exists: mockExists },
		);
		expect(result?.decision).toBe("block");
	...
	it("tronque le contenu si trop long", () => {
		...
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/gros.docx" } },
			{ exec: mockExec, exists: mockExists },
		);
	...
	it("laisse passer si la conversion échoue", () => {
		...
		expect(
			run(
				{ tool_name: "Read", tool_input: { file_path: "/tmp/bad.docx" } },
				{ exec: mockExec, exists: mockExists },
			),
		).toBeNull();
	});
	...
	it("laisse passer si le résultat est vide", () => {
		...
		expect(
			run(
				{ tool_name: "Read", tool_input: { file_path: "/tmp/empty.docx" } },
				{ exec: mockExec, exists: mockExists },
			),
		).toBeNull();
	});
	...
	it("supporte epub", () => {
		...
		const result = run(
			{ tool_name: "Read", tool_input: { file_path: "/tmp/livre.epub" } },
			{ exec: mockExec, exists: mockExists },
		);
		expect(result?.decision).toBe("block");
	});
