...
import {
	detectManager,
	run,
} from "../../.claude/hooks/file-changed-run-tests.mjs";
...
function makeDeps({
	...
	return {
		...
		exec: vi.fn(exec),
		exists: (p) =>
			(hasVitest && p.endsWith("/node_modules/.bin/vitest")) ||
			p.endsWith(`/${lockfile}`),
	};
}
...
describe("file-changed-run-tests", () => {
	...
	it("utilise vitest related sur le fichier modifié si vitest est présent", () => {
		...
		expect(deps.exec).toHaveBeenCalledWith(
			expect.stringContaining('vitest related --run "/fake/project/src/a.ts"'),
		);
	});
	...
	it("rapporte un échec", () => {
		const r = run(
			{ file_path: "a.ts" },
			makeDeps({ exec: makeExecFail("test failed") }),
		);
...
describe("detectManager", () => {
	const mk = (lockfile) => ({
		projectDir: "/p",
		exists: (p) => p.endsWith(`/${lockfile}`),
	});
	it("détecte pnpm", () =>
		expect(detectManager(mk("pnpm-lock.yaml"))).toBe("pnpm"));
