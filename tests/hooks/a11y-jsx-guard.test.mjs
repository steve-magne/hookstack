...
describe("a11y-jsx-guard — statique (fallback)", () => {
	it("ignore les fichiers hors src/", () => {
		expect(
			run(inp("/p/x.tsx"), { ...noBiome, readFile: () => "<Image src={x} />" }),
		).toBeNull();
	});

	it("ignore un fichier .ts (pas du JSX)", () => {
		expect(
			run(inp("/p/src/lib/utils.ts"), {
				...noBiome,
	...
	it("accepte les fichiers .jsx", () => {
		expect(
			run(inp("/p/src/components/Btn.jsx"), {
				...noBiome,
				readFile: () => "<Image src={x} />",
			})?.message,
		).toContain("alt");
	});
	...
	it("signale un <Image> sans alt", () => {
		expect(r("<Image src={x} width={10} />")?.message).toContain(
			"without an `alt`",
		);
	});
	...
	it("signale target=_blank sans rel noopener", () => {
		expect(r('<a href="/x" target="_blank">x</a>')?.message).toContain(
			'rel="noopener',
		);
	});

	it("signale onClick sur div sans role ni clavier", () => {
		expect(r("<div onClick={f}>x</div>")?.message).toContain(
	...
	it("accepte onClick sur div avec role + onKeyDown", () => {
		expect(
			r('<div onClick={f} role="button" onKeyDown={k}>x</div>'),
		).toBeNull();
	});
	...
	it("ne se laisse pas berner par un `>` dans une chaîne", () => {
		expect(
			r('<div title="a > b" onClick={f} role="button" onKeyDown={k}>x</div>'),
		).toBeNull();
	});
...
describe("a11y-jsx-guard — Biome (installé)", () => {
	...
	it("silencieux si la sortie n'est pas du JSON valide", () => {
		expect(
			run(inp(FILE_TSX), biomeDeps(makeExecFail("biome: command not found"))),
		).toBeNull();
	});
	...
	it("reporte une violation", () => {
		const r = run(
			inp(FILE_TSX),
			biomeDeps(
				makeExecFail(rdjson("lint/a11y/useAltText", "img needs alt", 5)),
			),
		);
	...
	it("cumule plusieurs violations", () => {
		const json = JSON.stringify({
			diagnostics: [
				{
					code: { value: "lint/a11y/useAltText" },
					message: "img needs alt",
					location: { range: { start: { line: 1 } } },
				},
				{
					code: { value: "lint/a11y/useKeyWithClickEvents" },
					message: "needs key",
