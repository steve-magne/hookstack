...
describe('protect-paths', () => {
  it('bloque .env', () => {
    expect(run({ tool_input: { file_path: '/proj/.env' } })?.decision).toBe('block');
  });

  it('bloque un fichier dans secrets/', () => {
    expect(run({ tool_input: { file_path: '/proj/secrets/key.txt' } })?.decision).toBe('block');
  });

  it('bloque une clé .pem', () => {
    expect(run({ tool_input: { file_path: '/proj/server.pem' } })?.decision).toBe('block');
  });
