...
describe('pre-write-secret-detection', () => {
  ...
  it('bloque une clé API style Anthropic dans content', () => {
    const r = run({ tool_name: 'Write', tool_input: { file_path: 'config.ts', content: `const k = "${SECRETS.anthropicKey}";` } });
    expect(r?.decision).toBe('block');
    expect(r?.reason).toContain('environment variable');
  });

  it('bloque un token GitHub dans new_string', () => {
    const r = run({ tool_name: 'Edit', tool_input: { file_path: 'ci.yml', new_string: `token: ${SECRETS.githubToken}` } });
    expect(r?.decision).toBe('block');
  });

  it('bloque une affectation de mot de passe', () => {
    expect(run({ tool_name: 'Write', tool_input: { file_path: 'db.py', content: SECRETS.passwordLine } })?.decision).toBe('block');
  });

  it('bloque une clé privée PEM', () => {
    expect(run({ tool_name: 'Write', tool_input: { file_path: 'key.pem', content: SECRETS.privateKey } })?.decision).toBe('block');
  });
