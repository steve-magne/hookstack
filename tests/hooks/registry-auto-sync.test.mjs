...
describe('registry-auto-sync', () => {
  ...
  it('resync sur édition du registre', () => {
    const exec = vi.fn(() => 'ok');
    run({ tool_input: { file_path: '/proj/registry/registry.json' } }, deps(exec));
    expect(exec).toHaveBeenCalled();
  });

  it('ignore les autres fichiers', () => {
    const exec = vi.fn();
    expect(run({ tool_input: { file_path: '/proj/src/x.ts' } }, deps(exec))).toBeNull();
    expect(exec).not.toHaveBeenCalled();
  });

  it('ne se resync pas lui-même', () => {
    const exec = vi.fn();
    expect(run({ tool_input: { file_path: '/proj/.claude/hooks/registry-auto-sync.mjs' } }, deps(exec))).toBeNull();
    expect(exec).not.toHaveBeenCalled();
  });
