...
describe("motion-rules-guard", () => {
	it("ignore les fichiers hors src/", () => {
		expect(
			run(input("/p/scripts/build.ts"), { readFile: () => "motion.div" }),
		).toBeNull();
	});

	it("ignore les fichiers non JS/TS", () => {
		expect(
			run(input("/p/src/app/globals.css"), {
				readFile: () => "prefers-reduced-motion",
			}),
		).toBeNull();
	});
	...
	it("signale un import framer-motion", () => {
		const r = run(input(FILE), {
			readFile: () => "import { motion } from 'framer-motion';",
		});
	...
	it("signale un élément motion.* (LazyMotion strict)", () => {
		const r = run(input(FILE), {
			readFile: () => "return <motion.div animate={{ opacity: 1 }} />;",
		});
	...
	it("signale une animation de width/height", () => {
		const r = run(input(FILE), {
			readFile: () => "<m.div animate={{ width: 300 }} />",
		});
