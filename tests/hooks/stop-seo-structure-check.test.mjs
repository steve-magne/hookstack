	...
	it("bloque (exitCode 2) si une page manque de métadonnées", () => {
		const out = run(
			{},
			{
				...base,
				readFile: (p) =>
					p.endsWith("page.tsx") ? "export default function P(){}" : "",
			},
		);
	...
	it("signale un lien interne cassé", () => {
		const out = run(
			{},
			{
				...base,
				readFile: (p) =>
					p.endsWith("page.tsx") ? META : '<Link href="/guide/x">x</Link>',
			},
		);
		...
		const out = run(
			{},
			{
				...
				readFile: (p) =>
					p.endsWith("page.tsx")
						? META
						: '<Link href="/">a</Link><Link href="/#catalogue">b</Link>',
			},
		);
		expect(out).toBeNull();
	});
