...
describe("quality-check", () => {
	...
	it("TypeScript tourne en incrémental (buildinfo) pour accélérer les runs suivants", () => {
		...
		expect(opts.exec).toHaveBeenCalledWith(
			expect.stringContaining(".tsbuildinfo"),
		);
	});
	...
	it("retourne failed=1 quand TypeScript échoue", () => {
		const err = Object.assign(new Error("type error"), {
			stdout: Buffer.from("Type error in foo.ts"),
		});
	...
	it("retourne failed>0 quand Biome échoue", () => {
		const err = Object.assign(new Error("lint error"), {
			stdout: Buffer.from("2 errors"),
		});
		const result = run(
			makeOpts({ hasBiomeConfig: true, execResults: { biome: err } }),
		);
		expect(result.failed).toBeGreaterThanOrEqual(1);
	...
	it("affiche le nombre de vérifications échouées dans le message", () => {
		...
		const result = run(
			makeOpts({ hasBiomeConfig: true, execResults: { tsc: err, biome: err } }),
		);
	...
	it("limite Biome aux fichiers JS/TS modifiés (pas tout le repo)", () => {
		...
		expect(opts.exec).not.toHaveBeenCalledWith(
			expect.stringMatching(/biome lint --error-on-warnings \.$/),
		);
	});
