...
describe('block-huge-write', () => {
  ...
  it('respecte un seuil injecté', () => {
    expect(run(write('x'.repeat(200)), { maxBytes: 100 })?.decision).toBe('block');
  });

  it('ignore Edit (patch ciblé, pas de contenu complet)', () => {
    expect(run({ tool_name: 'Edit', tool_input: { content: 'x'.repeat(600_000) } })).toBeNull();
  });

  it('laisse passer si content absent', () => {
    expect(run({ tool_name: 'Write', tool_input: { file_path: '/a' } })).toBeNull();
  });
});
