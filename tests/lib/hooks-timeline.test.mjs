...
const REGISTRY = [
	{
		...
		category: "security",
		implementation: { script_path: ".claude/hooks/detect-secrets.mjs" },
	},
	{
		slug: "load-git-context",
		name: "Load git context",
		category: "context",
		implementation: { script_path: ".claude/hooks/git-context.mjs" },
	},
];
...
describe("gitCreationDate", () => {
	...
	it("returns null when git throws", () => {
		expect(
			gitCreationDate(".claude/hooks/x.mjs", () => {
...
describe("renderHeatmapSvg", () => {
	...
	it("is a self-contained svg with a legend and an accessible label", () => {
		...
		expect(svg).toContain(
			'aria-label="HookStack — 1 hooks added since 2026-05-29"',
		);
...
describe("renderLinechartSvg", () => {
	...
	it("is a self-contained accessible SVG", () => {
		...
		expect(svg).toContain('role="img"');
		expect(svg).toContain(
			'aria-label="HookStack evolution — 3 hooks since 2026-05-29"',
		);
	});
...
describe("injectReadme", () => {
	...
	it("inserts the block before the first heading on first run", () => {
		...
		expect(out.indexOf("HOOKS_TIMELINE:START")).toBeLessThan(
			out.indexOf("## Promise"),
		);
	});
