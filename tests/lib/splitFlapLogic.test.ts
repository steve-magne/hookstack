...
describe('SplitFlap — logique de timing (mode normal)', () => {
  ...
  it('chaque char démarre après le précédent (cascade gauche → droite)', () => {
    for (let i = 1; i < 8; i++) {
      ...
      expect(charStateAt(i, prevLock, opts), `char ${i} pas encore démarré`).toBe('empty')
    }
  })
  ...
  it('le verrouillage progresse de gauche à droite : char i verrouillé avant char i+1', () => {
    for (let i = 0; i < 7; i++) {
      ...
      expect(charStateAt(i, lockI, opts), `char ${i} verrouillé`).toBe('locked')
      expect(charStateAt(i + 1, lockI, opts), `char ${i + 1} pas encore verrouillé`).not.toBe('locked')
    }
  })
...
describe('SplitFlap — logique de timing (mode eager)', () => {
  ...
  it('le verrouillage progresse gauche → droite après le délai', () => {
    for (let i = 0; i < 7; i++) {
      ...
      expect(charStateAt(i, lockI, opts), `char ${i} verrouillé`).toBe('locked')
      expect(charStateAt(i + 1, lockI, opts), `char ${i + 1} pas encore verrouillé`).toBe('glyph')
    }
  })
})
