...
const EMPTY: TimelineData = { total: 0, firstDate: null, lastDate: null, byDay: {}, hooks: [] }
...
describe('timelineStats', () => {
  ...
  it('handles an empty timeline without throwing', () => {
    expect(timelineStats(EMPTY)).toMatchObject({ total: 0, activeDays: 0, busiestCount: 0, longestStreak: 0 })
  })
})
...
describe('recentHooks', () => {
  ...
  it('breaks same-date ties alphabetically', () => {
    ...
    const sameDay = recent.filter((h) => h.date === '2026-05-29').map((h) => h.name)
