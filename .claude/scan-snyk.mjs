...
function runSnyk(dir) {
	try {
		...
		if (e.code === "ENOENT") {
			console.warn(
				"! snyk CLI not found — skipping (install snyk + set SNYK_TOKEN in CI).",
			);
			process.exit(0);
		}
		if (e.status === 1 && typeof e.stdout === "string") {
			try {
				return { sarif: JSON.parse(e.stdout), code: 1 };
			} catch {
				/* fall through */
			}
		}
		...
		console.error(
			`✗ snyk code test failed (exit ${e.status ?? "?"}): ${String(e.stderr || e.message).slice(0, 500)}`,
		);
		process.exit(1);
	}
}
...
function main(argv) {
	...
	try {
		...
		const flagged = Object.entries(fileCounts).filter(
			([, c]) => c.high || c.medium || c.low,
		);
		console.log(
			`✓ Scanned ${updated.length} hooks — ${flagged.length} with findings.`,
		);
		for (const [file, c] of flagged) {
			console.log(
				`   ${file}: ${c.high} high · ${c.medium} medium · ${c.low} low`,
			);
		}
