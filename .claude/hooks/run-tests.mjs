...
export function detect({
	...
	if (exists(pkg)) {
		try {
			...
			if (scripts.test) {
				const mgr = exists(join(projectDir, "pnpm-lock.yaml"))
					? "pnpm"
					: exists(join(projectDir, "bun.lockb")) ||
							exists(join(projectDir, "bun.lock"))
						? "bun"
						: exists(join(projectDir, "yarn.lock"))
							? "yarn"
							: "npm";
				...
				if (scoped && family === "vitest")
					return [mgr, ["test", "--", "--run", "--changed"]];
				if (scoped && family === "jest")
					return [mgr, ["test", "--", "--onlyChanged"]];
				return [mgr, ["test", "--", "--run"]];
			}
		} catch {}
	}
...
export function run({
	...
	if (result.status !== 0) {
		...
		const ok = affected
			? "✓ Tests liés aux changements passés"
			: "✓ Tests passés";
