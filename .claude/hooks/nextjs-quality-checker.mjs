...
export function run(
	input,
	{ readFile = readFileSync, fileExists = existsSync } = {},
) {
	...
	if (/<img\s/i.test(content) && !content.includes("next/image")) {
		warnings.push(
			`<img> tag detected — consider next/image for automatic optimization and lazy loading.`,
		);
	}

	// <a href> for internal paths (not http/mailto/tel)
	const internalAnchor =
		/<a\s[^>]*href=["'][^"'#]/.test(content) &&
		!/href=["']https?:/.test(content);
	if (internalAnchor && !content.includes("next/link")) {
		warnings.push(
			`Internal <a href> detected — use next/link for prefetching and client-side navigation.`,
		);
	}
