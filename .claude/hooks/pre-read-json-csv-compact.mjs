...
function inferJsonSchema(value, depth = 0) {
	...
	if (Array.isArray(value)) {
		const inner = value.length
			? inferJsonSchema(value[0], depth + 1)
			: "unknown";
	...
	if (typeof value === "object") {
		...
		const fields = keys.map(
			(k) => `${k}: ${inferJsonSchema(value[k], depth + 1)}`,
		);
		const extra =
			Object.keys(value).length > 12
				? ` … +${Object.keys(value).length - 12} more`
				: "";
...
function summarizeJsonl(raw, filePath) {
	...
	const preview = lines.slice(0, JSONL_PREVIEW_LINES).map((l) => {
		try {
			return inferJsonSchema(JSON.parse(l));
		} catch {
			return l.slice(0, 80);
		}
	});
...
export function run(
	input,
	{ readFile = readFileSync, exists = existsSync, stat = statSync } = {},
) {
	...
	if (ext === "json") summary = summarizeJson(raw, filePath);
		...
		summary = summarizeCsv(raw, filePath);
