...
const BIOME_UNAVAILABLE =
	/Cannot find module|could not determine executable|command not found|ENOENT/i;
...
export function run({
	...
	if (exists(disableFile)) {
		process.stderr.write(
			`[per-file-lint] SUSPENDU (≥3 échecs). rm '${disableFile}' pour réactiver.\n`,
		);
		return { exitCode: 0 };
	}
	...
	if (!failed.length) {
		...
		return {
			exitCode: 0,
			message: checked
				? `✓ Biome: no issues (${checked} file${checked > 1 ? "s" : ""})\n`
	...
	let count = 0;
	try {
		count = parseInt(readFile(counterFile, "utf8").trim(), 10) || 0;
	} catch {}
	count++;
