...
function defaultWalkTsx(
	...
	for (const entry of readdir(dir, { withFileTypes: true })) {
		...
		if (entry.isDirectory())
			out.push(...defaultWalkTsx(full, { exists, readdir }));
		else if (entry.name.endsWith(".tsx")) out.push(full);
	}
	return out;
}

function defaultListDirs(
	dir,
	{ exists = existsSync, readdir = readdirSync } = {},
) {
	if (!exists(dir)) return [];
...
function hasMetadata(content) {
	...
	return (
		present && /\btitle\s*:/.test(content) && /\bdescription\s*:/.test(content)
	);
}
...
export function run(_input, deps = {}) {
	...
	for (const page of pages) {
		if (!hasMetadata(read(page))) {
			problems.push(
				`metadata missing (title + description) → ${page.replace(`${projectDir}/`, "")}`,
			);
		}
	}
	...
	const need = [
		[
			"robots",
			["src/app/robots.ts", "src/app/robots.txt", "public/robots.txt"],
		],
		[
			"sitemap",
			["src/app/sitemap.ts", "src/app/sitemap.xml", "public/sitemap.xml"],
		],
		[
			"OpenGraph image",
			[
	...
	for (const [label, candidates] of need) {
		if (!candidates.some((c) => exists(join(projectDir, c)))) {
			problems.push(
				`${label} missing → expected one of: ${candidates.join(", ")}`,
			);
		}
	}
	...
	for (const file of walkTsx(join(projectDir, "src"))) {
		...
		for (const [, href] of content.matchAll(LINK_RE)) {
			...
			problems.push(
				`broken internal link "${href}" → unknown route in ${file.replace(`${projectDir}/`, "")}`,
			);
		}
	}
