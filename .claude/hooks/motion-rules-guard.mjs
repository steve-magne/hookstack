...
const RULES = [
	[
		...
		"import 'framer-motion' → use 'motion/react' (paquet `motion`)",
	],
	[
		/<motion\.\w+/,
		'<motion.*> crashes under LazyMotion strict → use <m.*> (import { m } from "motion/react")',
	],
	[
		/prefers-reduced-motion/,
		'manual prefers-reduced-motion query → remove it, MotionConfig reducedMotion="user" handles it globally',
	],
	[
		/animate=\{\{[^}]*\b(?:width|height|top|left)\s*:/s,
		"animating width/height/top/left → use transform (x, y, scale) or the `layout` prop",
	],
];
...
export function run(input, { readFile = readFileSync } = {}) {
	...
	const violations = RULES.filter(([pattern]) => pattern.test(content)).map(
		([, fix]) => fix,
	);
