...
export async function fetchCodeqlAlerts(repo, token, { fetch: _fetch = fetch } = {}) {
  const alerts = []
  let page = 1
  while (true) {
    ...
    if (res.status === 404) {
      console.warn('! Code Scanning not enabled on this repo or no alerts found.')
      return []
    }
    if (!res.ok) {
...
export function attachVerdicts(hooks, fileCounts, scannedAt) {
  ...
  for (const hook of hooks) {
    ...
    const found = fileCounts[file] ?? { critical: 0, high: 0, medium: 0, low: 0 }
...
async function main(argv) {
  ...
  console.log(`  ${alerts.length} open CodeQL alert${alerts.length === 1 ? '' : 's'} found.`)

  const fileCounts = aggregateAlerts(alerts)
  const scannedAt = new Date().toISOString()
  const updated = attachVerdicts(hooks, fileCounts, scannedAt)

  const flagged = Object.entries(fileCounts).filter(([, c]) =>
    c.critical || c.high || c.medium || c.low,
  )
  console.log(`✓ ${updated.length} hooks updated — ${flagged.length} with findings.`)
  for (const [file, c] of flagged) {
    console.log(`   ${file}: ${c.critical} critical · ${c.high} high · ${c.medium} medium · ${c.low} low`)
  }
