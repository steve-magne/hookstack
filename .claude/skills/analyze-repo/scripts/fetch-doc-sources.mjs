...
if (!url) {
	process.stdout.write(
		`${JSON.stringify({ error: "Usage: fetch-doc-sources.mjs <url> [registry.json]" })}\n`,
	);
	process.exit(0);
}
...
function focusOnHookContent(text) {
	...
	const scored = lines.map((line, i) => {
		...
		const score = hookKeywords.reduce(
...
try {
	...
	if (!res.ok) {
		process.stdout.write(
			`${JSON.stringify({ error: `HTTP ${res.status} pour ${url}` })}\n`,
		);
		process.exit(0);
	}
	...
	let existingSlugs = [];
	try {
		existingSlugs = JSON.parse(readFileSync(registryPath, "utf8")).map(
			(h) => h.slug,
		);
	} catch {}
