...
export function NotFoundScene3D() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      const nodes = positions.map((position, index) => {
        const node = new THREE.Mesh(nodeGeometry, index === 3 ? hotMaterial : nodeMaterial)
      ...
      const packet = new THREE.Mesh(new THREE.OctahedronGeometry(0.24, 1), packetMaterial)
      ...
      runtimeRef.current = { renderer, scene, camera, rig, nodes, packet, severedLine }
  ...
  useAnimationFrame((time) => {
    ...
    runtime.packet.position.set(0.55 + orbit, -0.62 + Math.cos(t * 1.2) * 0.08, 0.45)

    runtime.nodes.forEach((node, index) => {
      const pulse = reduceMotion ? 1 : 1 + Math.sin(t * 2.4 + index * 0.85) * 0.055
      node.scale.setScalar(pulse)
      node.rotation.y = pace * 0.35 + index
    })

    const lineMaterial = runtime.severedLine.material
    if ('opacity' in lineMaterial) lineMaterial.opacity = reduceMotion ? 0.58 : 0.5 + Math.sin(t * 3.2) * 0.22
