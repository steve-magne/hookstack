...
import { CATEGORY_COLORS, CATEGORY_LABELS } from "@/types/hook";
		...
		>
			...
			<m.header variants={fadeUp} className="mb-10">
				...
				<p className="max-w-2xl text-lg text-zinc-400">
					Every hook in the catalogue is dogfooded on this very repository,
					unit-tested, and shipped in public. Here is the whole journey — pulled
					straight from git history, never hand-edited.
				</p>
			</m.header>
...
function GrowthCurve({
	...
	const linePath = series
		.map(
			(p, i) =>
				`${i === 0 ? "M" : "L"} ${x(p.date).toFixed(1)} ${y(p.cumulative).toFixed(1)}`,
		)
		.join(" ");
