...
import type { GroupBy } from "./HookRow";
							...
							{options.map((opt) => {
								...
								return (
									...
									>
										...
										<span
											className={`text-xs transition-colors ${isSelected ? "text-white" : "text-zinc-400"}`}
										>
											{opt.label}
										</span>
...
export function CatalogueExplorer({
	...
	const categoryOptions: FilterOption[] = useMemo(
		() =>
			CATEGORY_ORDER.filter((cat) => (categoryCounts[cat] ?? 0) > 0).map(
				(cat) => ({
					value: cat,
	...
	const toggleCategory = useCallback((cat: string) => {
		...
		setSelectedCategories((prev) =>
			prev.includes(cat as Category)
				? prev.filter((x) => x !== cat)
				: [...prev, cat as Category],
		);
	}, []);
	...
	const windowLabels = useMemo<Record<RecencyWindow, string>>(
		() => ({
			week: T.recentThisWeek,
			month: T.recentThisMonth,
			earlier: T.recentEarlier,
		}),
		[T],
	);
		...
		<div data-component="CatalogueExplorer">
			...
			>
				...
				<div className="flex items-center gap-3">
					...
					{selectedSlugs.length > 0 && (
						<span className="hidden shrink-0 rounded-full border border-zinc-700/70 bg-zinc-900/50 px-3 py-1 font-mono text-xs text-zinc-400 sm:inline-block">
							<span className="text-zinc-100">{selectedSlugs.length}</span> /{" "}
							{allHooks.length}
						</span>
					)}
				</div>
				...
				<div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
					{!initialCategory && (
						<GroupBySegmented value={groupBy} onChange={changeGroupBy} />
					)}

					<div
						className="hidden h-4 w-px bg-zinc-700/50 sm:block"
						aria-hidden
					/>
			...
			{/* Grouped list — les trois modes partagent les sections deux colonnes
					façon Vercel (EventSections) : description à gauche, tuiles à droite,
					même gestion de scroll. La colonne gauche s'adapte au mode. */}
			{results.length > 0 ? (
				<EventSections
					groups={groups}
					mode={groupBy}
