...
import { MAINTAINER, PERSON_SAME_AS, SAME_AS, SITE } from "@/lib/site";
...
async function getStars(): Promise<number | null> {
	try {
		const res = await fetch(
			"https://api.github.com/repos/steve-magne/hookstack",
			{
...
export default async function HomePage() {
	...
	const webPageJsonLd = {
		...
		speakable: {
			...
			cssSelector: [
				"#faq-heading",
				'[data-component="FaqSection"] dt',
				'[data-component="FaqSection"] dd',
			],
		},
	};
		...
		<>
			...
			<div data-component="HomePage" className="mx-auto max-w-6xl px-4">
				...
				<section
					...
					className="relative pt-20 pb-12 text-center sm:pt-28"
				>
							...
							<tr className="bg-white/5">
								{T.compareCols.map((c) => (
									<th
										key={c || "dim-col"}
										scope="col"
										className="p-3 font-semibold text-zinc-300"
									>
										{c}
									</th>
								))}
			...
			>
				...
				>
					...
					>
						...
						<span
							aria-hidden
							className="text-zinc-500 transition-transform group-hover:translate-x-0.5 group-hover:text-[var(--color-brand)]"
						>
							→
						</span>
					</Link>
			...
			>
				<h2 id="faq-heading" className="mb-10 text-xl font-semibold text-white">
					{T.faqTitle}
				</h2>
