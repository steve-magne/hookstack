...
export default function NotFoundPage() {
  return (
    <div data-component="NotFoundPage" className="mx-auto grid min-h-[calc(100vh-8rem)] max-w-6xl items-center gap-10 px-4 py-10 lg:grid-cols-[0.92fr_1.08fr] lg:py-16">
      <section className="relative min-w-0">
        ...
        <p className="mt-6 max-w-xl text-pretty text-base leading-relaxed text-zinc-300 sm:text-lg">
          This URL did not match a published hook, but the catalogue is still wired.
          Rejoin the index, inspect the source, or keep the install flow moving.
        </p>
        ...
        <div className="mt-9 overflow-hidden rounded-xl border border-zinc-700/70 bg-[#0c0c12] shadow-lg shadow-black/40">
          <div className="flex items-center gap-2 border-b border-white/5 bg-white/[0.015] px-3.5 py-2">
            ...
            <span className="ml-1 font-mono text-[11px] text-zinc-500">diagnostics</span>
          </div>
          <dl className="grid gap-px bg-white/5 sm:grid-cols-2">
            {diagnostics.map(([key, value]) => (
              <div key={key} className="bg-[#0c0c12] px-4 py-3">
                <dt className="font-mono text-[10px] uppercase tracking-wide text-zinc-600">{key}</dt>
