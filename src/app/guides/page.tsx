...
export default function GuidesIndexPage() {
  ...
  return (
    <>
      ...
      {/* GuidesIndexPage */}
      <div data-component="GuidesIndexPage" className="mx-auto max-w-3xl px-4 py-12">
        <h1 className="mb-4 text-3xl font-bold text-white">Claude Code Hooks Guides</h1>
        ...
        <ul className="space-y-4">
          {guides.map((g) => (
            <li key={g.slug}>
              ...
              >
                <h2 className="mb-1 text-lg font-semibold text-white">{g.title}</h2>
                <p className="mb-2 text-sm leading-relaxed text-zinc-400">{g.description}</p>
                <span className="text-xs uppercase tracking-wide text-zinc-500">{g.readingMinutes} min read</span>
              </Link>
            </li>
          ))}
        </ul>
