			...
			{parts.map((p, i) =>
				i % 2 === 1 ? (
					<code
						...
						key={i}
						className="rounded bg-white/10 px-1 py-0.5 text-[0.85em] text-zinc-200"
					>
						{p}
					</code>
				) : (
...
export default async function GuidePage({
	...
	const articleJsonLd = {
		...
		publisher: {
			...
			logo: {
				"@type": "ImageObject",
				url: `${SITE.base}/web-app-manifest-512x512.png`,
			},
		},
	};
	...
	const breadcrumbJsonLd = {
		...
		itemListElement: [
			...
			},
			{
				...
				position: 3,
				name: guide.title,
				item: `${SITE.base}/guides/${guide.slug}`,
			},
		],
	};
