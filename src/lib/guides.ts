						...
						code: `// .claude/settings.json
{
  ...
  "tool_input": {
    "command": "rm -rf / --no-preserve-root"
		...
		faq: [
			{
				q: "Do Claude Code hooks have to be written in JavaScript?",
				a: "No. A hook is any executable command — Bash, Python, or a binary all work. HookStack standardizes on Node.js .mjs files because Node is the one runtime Claude Code guarantees on every platform, so the same hook runs on macOS, Linux, and Windows.",
			},
			{
				q: "Do hooks consume tokens from my context window?",
				a: "Only what they print back. A hook runs in a separate process; nothing it computes reaches the model unless it returns text — a blocking reason or context injected by a UserPromptSubmit hook. A silent hook costs zero tokens.",
			},
			{
				q: "Can a hook block Claude from doing something?",
				a: 'Yes — a PreToolUse hook can block the action by writing { "decision": "block", "reason": "…" } to stdout before the tool runs.',
			},
			{
				q: "Where do hooks live in my project?",
				a: 'Scripts live in .claude/hooks/ and are referenced by event and matcher in .claude/settings.json, usually as "node $CLAUDE_PROJECT_DIR/.claude/hooks/<name>.mjs".',
			},
		],
		related: [
			...
			"claude-code-hooks-vs-slash-commands",
			"inject-context-claude-code-hooks",
		],
		relatedHookSlugs: [
...
{ "hook_event_name": "PostToolUse", "tool_name": "Write",
  ...
  "tool_response": "File written successfully" }`,
		...
		faq: [
			{
				q: "Is PreToolUse a blocking hook?",
				a: 'Yes. PreToolUse runs before the tool and can block it by returning { "decision": "block", "reason": "…" } on stdout. PostToolUse runs after the change has landed, so it cannot prevent it.',
			},
			{
				q: "What is the difference in the stdin payload?",
				a: "Both get tool_name and tool_input. PostToolUse additionally receives tool_response — the result of the tool that just ran — which is why it can react to outcomes.",
			},
			{
				q: "Are PostToolUse hooks always non-blocking?",
				a: "Not inherently. HookStack PostToolUse hooks exit quietly on missing tools by convention (silent try/catch), but the runtime still lets a PostToolUse hook return a block decision. The non-blocking behavior is a design choice, not a hard rule.",
			},
			{
				q: "Can I use both events together?",
				a: "Yes, and it is a common combination — PreToolUse to guard, PostToolUse to format and check.",
			},
		],
		related: [
			"what-are-claude-code-hooks",
			"secure-claude-code-with-hooks",
			"claude-code-hooks-not-working",
			"write-your-first-claude-code-hook",
			"automate-code-quality-claude-code-hooks",
		],
		relatedHookSlugs: [
    ...
    "Stop": [
      {
		...
		faq: [
			{
				q: "Are hooks better than slash commands?",
				a: "They do different jobs. Slash commands are interactive prompts you trigger; hooks are commands the runtime fires automatically and the model cannot skip. Use both.",
			},
			{
				q: "Do hooks replace CLAUDE.md?",
				a: "No. CLAUDE.md is great for soft guidance the model interprets. Hooks enforce the rules that must hold regardless of what the model decides.",
			},
			{
				q: "Do hooks really cost zero tokens?",
				a: "A silent hook does — it runs outside the context window. But any text a hook prints back (a block reason, injected context) does consume tokens. You control exactly what reaches the model.",
			},
			{
				q: "What is the difference between a hook and an MCP server?",
				a: "An MCP server adds new capabilities and data the model can use. A hook governs the lifecycle around tools the model already has — allowing, blocking, logging, and automating.",
			},
		],
		related: [
			"what-are-claude-code-hooks",
...
{ "type": "command", "command": "node $CLAUDE_PROJECT_DIR/.claude/hooks/x.mjs" }`,
			...
			},
			{
				...
				body: [
					{
						list: [
							"Run `claude --debug` and confirm the hook is matched and executed.",
							"Pipe a fake payload to the script directly — does it print the JSON you expect?",
							"Validate `.claude/settings.json` parses as JSON.",
							"Confirm the event name and matcher are correct and case-exact.",
							"Confirm decisions go to stdout and logs go to stderr.",
							"Confirm the command uses `$CLAUDE_PROJECT_DIR`.",
		...
		faq: [
			{
				q: "How do I debug a Claude Code hook?",
				a: 'Run Claude Code with claude --debug to see hooks being matched and executed, and test the script directly by piping it a fake payload: echo \'{"tool_name":"Bash","tool_input":{"command":"…"}}\' | node .claude/hooks/your-hook.mjs.',
			},
			{
				q: "Why is my hook firing but not blocking?",
				a: 'Claude Code only reads a block decision from valid JSON on stdout. A stray console.log corrupts stdout, the wrong shape (not { decision: "block", reason }) is ignored, and logs must go to stderr, not stdout.',
			},
			{
				q: "What exit code should a Claude Code hook return?",
				a: "Exit 0 and print a JSON decision to stdout (the HookStack convention). Exit 2 blocks the action and sends stderr to Claude. Any other code is a non-blocking error and the action proceeds — which is what happens when your hook crashes.",
			},
			{
				q: "Why does my hook work in the terminal but not in Claude Code?",
				a: "Almost always a path issue. Reference the script with $CLAUDE_PROJECT_DIR (node $CLAUDE_PROJECT_DIR/.claude/hooks/x.mjs) instead of a relative path, because Claude Code may run from a different working directory.",
			},
		],
		related: [
			"write-your-first-claude-code-hook",
		...
		sources: [
			{ label: "Anthropic — Claude Code hooks documentation", url: DOCS },
			{
				label: "Anthropic — Claude Code hooks reference (exit codes & output)",
				url: DOCS,
			},
		],
	},
...
# {"ts":"2026-06-12T…","cmd":"git status"}`,
  ...
  return null
		...
		faq: [
			{
				q: "Where do I put my Claude Code hook?",
				a: "Put the script in .claude/hooks/ at your project root, then register it in .claude/settings.json under the right event (PreToolUse, PostToolUse, Stop, etc.) with a command like node $CLAUDE_PROJECT_DIR/.claude/hooks/your-hook.mjs.",
			},
			{
				q: "Do I need to install anything to write a hook?",
				a: "No. Hooks are plain Node.js scripts using only built-in modules (fs, path, child_process). Node is already required by Claude Code, so there is nothing extra to install.",
			},
			{
				q: "How do I test a hook before using it?",
				a: 'Pipe it a fake payload: echo \'{"tool_name":"Bash","tool_input":{"command":"git status"}}\' | node .claude/hooks/your-hook.mjs and check the output or side effect. This isolates the script from the session.',
			},
			{
				q: "How do I make my hook block an action instead of just reacting?",
				a: 'Use the PreToolUse event and return { decision: "block", reason: "…" } from run(), then write that JSON to stdout in the entry guard. PostToolUse runs after the action, so it cannot block.',
			},
		],
		related: [
			"what-are-claude-code-hooks",
		...
		relatedHookSlugs: [
			"pre-bash-block-destructive",
			"pre-bash-enforce-package-managers",
			"pre-bash-secret-detection",
		],
		sources: [
			{ label: "Anthropic — Claude Code hooks documentation", url: DOCS },
  ...
  "model": "claude-opus-4-8"
		...
		faq: [
			{
				q: "Where is the Claude Code settings.json file?",
				a: "Project settings live in `.claude/settings.json` at the repository root, personal gitignored overrides in `.claude/settings.local.json`, and global defaults in `~/.claude/settings.json` in your home directory.",
			},
			{
				q: "What is the difference between settings.local.json and settings.json?",
				a: "`settings.json` is the committed, team-shared config; `settings.local.json` is a gitignored personal layer that overrides it. Local wins over project, and project wins over the user-level file.",
			},
			{
				q: "Why are my Claude Code hooks defined in settings.json not running?",
				a: "The most common cause is invalid JSON — a trailing comma or missing brace makes the file unparseable and silently ignored. Validate it with a JSON parser, and make sure command paths use `$CLAUDE_PROJECT_DIR`.",
			},
			{
				q: "Should I commit settings.json to git?",
				a: "Yes, commit `.claude/settings.json` so the team shares the same hooks and permissions. Keep secrets and personal tweaks in `.claude/settings.local.json`, which should be gitignored.",
			},
		],
		related: [
			"what-are-claude-code-hooks",
		...
		relatedHookSlugs: [
			...
			"instructions-loaded-audit-log",
			"permission-request-auto-allow-readonly",
			"user-prompt-inject-conventions",
		],
		sources: [
			{ label: "Anthropic — Claude Code hooks documentation", url: DOCS },
    ...
    "PreToolUse": [
      {
		...
		faq: [
			{
				q: "What are the best Claude Code hooks to start with?",
			...
			},
			{
				...
				a: "Well-written hooks are negligible. `PreToolUse` hooks do cheap pattern matching, and `PostToolUse` tools like Biome should filter by file extension so they only run on relevant files. Always set an explicit timeout so a hook can never hang the session.",
			},
			{
				q: "Can a hook actually stop Claude from doing something?",
				a: "Yes, but only `PreToolUse` hooks. They run before a tool executes and can return `{ decision: 'block', reason: '…' }` on stdout to reject the call. `PostToolUse`, `Stop`, and `SessionStart` hooks react or inject context but cannot retroactively undo a completed action.",
			},
			{
				q: "Where do installed hooks live?",
				a: "Scripts go in your project’s `.claude/hooks/` directory and are referenced from `.claude/settings.json` via `$CLAUDE_PROJECT_DIR`. The HookStack CLI writes both, so the script on disk and its registration stay in sync.",
			},
		],
		related: [
			"what-are-claude-code-hooks",
			...
			"secure-claude-code-with-hooks",
			"automate-code-quality-claude-code-hooks",
			"claude-code-seo-accessibility-nextjs-hooks",
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (result) process.stdout.write(JSON.stringify(result))
		...
		faq: [
			{
				q: "Can the model disable or skip a PreToolUse hook?",
				a: "No. Hooks are configured in `settings.json` and executed by Claude Code itself, not by the model. A `PreToolUse` guard runs before every matching tool call, and its `block` decision is final regardless of what the model intends.",
			},
			{
				q: "Is a CLAUDE.md instruction enough to keep the agent safe?",
				a: "No. Instructions in `CLAUDE.md` are probabilistic context the model may misread or deprioritize. Use it to express intent, but enforce hard limits with deterministic `PreToolUse` hooks that run every time.",
			},
			{
				q: "Do these hooks replace code review?",
				a: "No. Hooks are defense-in-depth: they block the dangerous patterns you anticipated. You still need to review what the agent changed, because no pattern list covers every way an action can go wrong.",
			},
			{
				q: "Will secret-detection hooks slow down my session?",
				a: "No meaningfully. The hooks filter on `tool_name` first and run a handful of regular expressions against a single payload, so the overhead per tool call is negligible compared to the model and tool latency.",
			},
		],
		related: [
			"what-are-claude-code-hooks",
    ...
    "Stop": [
      {
		...
		faq: [
			{
				q: "Can a PostToolUse hook break my Claude Code session if a tool is missing?",
				a: "Not if it is written correctly. HookStack quality hooks wrap external tools in a silent try/catch so a missing formatter or linter exits quietly and the session continues. The hook returns null and control passes through.",
			},
			{
				q: "How do I run tests automatically in Claude Code?",
				a: "Register a Stop hook that runs your test command. The Stop event fires every time Claude finishes a turn. Set HOOKSTACK_TEST_CMD to your command (pnpm test, pytest, cargo test) and use exit code 2 to feed test failures back to Claude as context.",
			},
			{
				q: "Should I run tsc after every file write?",
				a: "Better to use a PostToolBatch hook: it fires after a group of edits, checks the full project once, and skips entirely if no TypeScript files were touched. Running tsc on every single write is slow — one pass per batch is enough.",
			},
			{
						...
						code: `// stdin payload received by a UserPromptSubmit hook
{
  ...
  "prompt": "refactor the auth module"
			...
			{
				heading:
					"How do you inject conventions and the current date on every prompt?",
				body: [
    ...
    "UserPromptSubmit": [
      {
        "hooks": [
          { "type": "command", "command": "node $CLAUDE_PROJECT_DIR/.claude/hooks/user-prompt-inject-conventions.mjs" }
			...
			{
				heading:
					"How do you inject git state at session start with SessionStart?",
				body: [
    ...
    "SessionStart": [
      {
        "hooks": [
          { "type": "command", "command": "node $CLAUDE_PROJECT_DIR/.claude/hooks/session-start-load-git-context.mjs" }
			...
			{
				heading:
					"How does the SessionStart source field let context survive compaction?",
				body: [
  ...
  // ... same git context logic as the full hook above
	...
	},
	{
		...
		sections: [
			...
			{
				heading:
					"How do you enforce title + description on every Next.js page?",
				body: [
  ...
  "hooks": {
	...
	},
	{
		...
		sections: [
			...
			{
				heading:
					"How do you redirect the agent to a worktree if it starts on main?",
				body: [
    ...
    "SessionStart": [
      {
        "hooks": [
          { "type": "command", "command": "node $CLAUDE_PROJECT_DIR/.claude/hooks/session-start-worktree-if-main.mjs" }
			...
			{
				heading:
					"How do you provision a new worktree with environment files and dependencies?",
				body: [
    ...
    "SessionStart": [
      {
        "hooks": [
          ...
          { "type": "command", "command": "node $CLAUDE_PROJECT_DIR/.claude/hooks/worktree-create-update-deps.mjs" }
			...
			{
				heading:
					"How do you prevent an edit from leaking into the main repository?",
				body: [
  ...
  "hooks": {
    "WorktreeRemove": [
      {
		...
		relatedHookSlugs: [
			"session-start-worktree-if-main",
			"worktree-create-setup-env",
			"worktree-create-update-deps",
			"pre-edit-worktree-guard",
	...
	},
	{
		...
		sections: [
			...
			{
				heading:
					"Why run Python tools via `uv run` instead of calling them directly?",
				body: [
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  if (result) process.stdout.write(JSON.stringify(result))
		...
		faq: [
			{
				q: "Do OpenAI Codex hooks use the same scripts as Claude Code?",
				a: "Yes. Codex and Claude Code share the same lifecycle event names and stdin payload shape, so the hook script (.mjs) is byte-for-byte identical. Only the config file differs: Claude Code uses .claude/settings.json with a top-level hooks key, Codex uses .codex/hooks.json with events at the root.",
			},
			{
				q: "Where does the Codex hooks config file live?",
				a: "In .codex/hooks.json — either in your project root (per-project) or in ~/.codex/hooks.json (per-profile, applies to every project). Scripts live in .codex/hooks/.",
			},
			{
				q: "How do I install Codex hooks?",
				a: "Run npx hookstack-cli@latest install --codex-project from your project root (or --codex-profile for the home-level config). The CLI writes the scripts and generates the Codex-format hooks.json for you.",
			},
			{
				q: "Can a Codex hook block the agent from running a command?",
				a: 'Yes — a PreToolUse hook writes { "decision": "block", "reason": "…" } to stdout before the tool runs, exactly as it does in Claude Code.',
			},
		],
		related: [
			"what-are-claude-code-hooks",
		...
		sources: [
			{
				label:
					"Anthropic — Claude Code hooks documentation (shared lifecycle events)",
				url: DOCS,
			},
