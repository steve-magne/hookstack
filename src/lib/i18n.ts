...
export const T = {
	...
	worksWithCaption:
		"Same hooks, one CLI — portable across agents that share Claude Code lifecycle events.",
	...
	compareRows: [
		{
			dim: "How it runs",
			hook: "Node.js script, outside the model",
			cmd: "You type it; the model interprets",
			prompt: "Text the model may follow",
		},
		{
			dim: "Deterministic?",
			hook: "Yes — fires unconditionally",
			cmd: "No — the model decides",
			prompt: "No — probabilistic",
		},
		{ dim: "Can it be skipped?", hook: "No", cmd: "Yes", prompt: "Yes" },
		{
			dim: "Token cost",
			hook: "Zero (separate process)",
			cmd: "Consumes context",
			prompt: "Consumes context",
		},
		{
			dim: "Best for",
			hook: "Guardrails, gates, automation",
			cmd: "On-demand interactive tasks",
			prompt: "Soft guidance & style",
		},
	],
