 ...
 */
export const LEVEL_COLORS = ['#1c1c20', '#3f3f46', '#71717a', '#a1a1aa', '#f4f4f5'] as const
...
export function buildWeeks(data: TimelineData = timeline, minWeeks = MIN_WEEKS): Cell[][] {
  ...
  const weekCount = Math.floor((end.getTime() - start.getTime()) / (7 * DAY)) + 1
...
export function cumulativeSeries(data: TimelineData = timeline): CumulativePoint[] {
...
export function monthLabels(weeks: Cell[][]): { label: string; weekIndex: number }[] {
...
export function recentHooks(n = 8, data: TimelineData = timeline): TimelineHook[] {
  return [...data.hooks]
