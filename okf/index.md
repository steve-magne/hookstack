...
* [Harness E2E du CLI — install/update/contribute contre un registre local](implementation/cli-e2e-harness.md) - Nouveau tests/cli/e2e.test.mjs : un serveur HTTP local sert le vrai registry.json dans la forme de l'API, et des dépôts git scratch exécutent le CLI réel (install/update/contribute) en mode non-interactif. Plus de coverage automatisée des flows cli.mjs sans réseau ni compte GitHub.
...
* [CLI — option --github-action + mode full-check des hooks de gate](implementation/cli-github-action.md) - Le CLI propose désormais d'écrire une GitHub Action (.github/workflows/hookstack-gates.yml) qui rejoue les mêmes gates qualité/tests qu'en session agentique et en pre-commit. Pour que les hooks tournent en CI (arbre git propre), ils gagnent un mode HOOKSTACK_FULL_CHECK=1 (check complet).
...
* [Nouveau hook stop-mobile-ui-test-gate — contribution généralisée depuis un projet consommateur](implementation/hook-mobile-ui-test-gate.md) - Ajout au catalogue d'un hook Stop qui bloque la fin de session quand un écran/composant React Native change sans son test de régression. Découvert déjà écrit (avec fingerprint @hookstack) dans un projet consommateur sans jamais avoir été poussé via `contribute` — généralisé (chemin monorepo en dur retiré) avant intégration.
...
* [Helper commun « spawn env » — cache npm relocalisé pour les spawns sandboxés](implementation/hooks-spawn-env.md) - Les hooks qui spawnt un gestionnaire de paquets (npx, npm, pnpm, yarn) mouraient en EPERM avant d'avoir lancé la moindre commande — npx écrit dans ~/.npm par défaut, inaccessible en écriture depuis une sandbox d'agent. Correction structurelle : module partagé lib/spawn-env.mjs (hookSpawnEnv) posant NPM_CONFIG_CACHE=<repo>/.npm-cache-tmp, appliqué aux 11 hooks concernés.
* [i18n — détection et validation de l'ensemble des emplacements standards](implementation/i18n-standard-locations.md) - Le signal i18n du CLI et le hook stop-i18n-validation couvrent désormais tous les emplacements et formats de traduction imposés par les écosystèmes (gettext, Android, Apple, Flutter, Java, Qt) et pas seulement les JSON sous locales/messages/i18n.
* [Support de la stack Java — hooks spécialisés + détection CLI](implementation/java-stack-support.md) - La stack `java` rejoint `typescript`/`python` : 4 hooks spécialisés (format google-java-format, compile, tests, enforce wrapper), détection par manifeste (pom.xml/build.gradle/gradlew) et filtrage par défaut côté CLI.
