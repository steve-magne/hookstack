...
`package.json`/`tsconfig.json`/`pnpm-workspace.yaml` → typescript ; `pyproject.toml`/`requirements.txt`/`setup.py`/`Pipfile`/`uv.lock` → python ; flags `--stack`/`--stacks` pour surcharger, `--no-detect` pour désactiver). **Toolchain inconnue** (Go, Rust, projet sans manifeste) : la détection n'exclut plus rien silencieusement — seuls les hooks **universels** (sans `stack`) sont installés, avec un message listant les slugs écartés. Depuis 2026-08 aussi, les 3 gardes SEO Next.js-only (`seo-page-metadata-guard`, `seo-next-image-guard`, `stop-seo-structure-check`) ont quitté le set `default_on` : elles ne sont proposées qu'aux projets détectés `nextjs` via `AUTO_DETECT` (voir [cli-smart-toolstack-detection](/implementation/cli-smart-toolstack-detection.md)). Sur un projet Python, un hook avec une **variante Python** (`implementation.python_script_path` + `python_code_snippet` + `python_test_snippet`) est installé en `.py` (commande `python3`) avec des tests **pytest** — jamais de vitest. Les hooks sans variante retombent en `.mjs` (fallback listé dans le résumé). Détails : [/implementation/python-hook-variants](/implementation/python-hook-variants.md).
