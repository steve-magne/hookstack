...
- **Hookstack est multi-agent** : le CLI installe les mêmes hooks pour Claude Code (`.claude/settings.json`), OpenAI Codex (`.codex/hooks.json`, flags `--codex-project`/`--codex-profile`) et GitHub Copilot (`--copilot`). Le code `.mjs` est identique ; seul le format de config diffère. Positionnement : *le catalogue de hooks agentiques, agnostique de l'agent*.
