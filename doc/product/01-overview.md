...
Dans tous les cas, le CLI installe les `.mjs` et patche le fichier de config de l'agent ciblé. **Le CLI est language-aware** : il détecte la toolstack du projet (TypeScript/Node via `package.json`/`tsconfig.json` ; Python via `pyproject.toml`/`requirements.txt`/…) et n'installe que les hooks compatibles — un projet Python reçoit la stack ruff/pyright/pytest via `uv`, jamais des hooks qui appellent `npm`/`tsc`/`biome`. Un repo mixte reçoit les deux ; un repo sans toolchain reconnue (Rust, Go…) ne reçoit que les hooks universels. Surcharge possible via `--stack=typescript|python|all`. Le menu interactif demande l'agent cible (ordre : This project → All my projects → Codex profile → Codex project → GitHub Copilot), ou on le choisit via un flag :
