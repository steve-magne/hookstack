...
**Hooks actifs** : voir `.claude/settings.json` (généré par sync). 94 hooks du catalogue sont actifs sur ce projet, chacun avec un test dans `tests/hooks/`. Pour consulter la liste complète : `node .claude/sync-hooks.mjs --dry-run`. Certains hooks du catalogue sont volontairement **exclus localement** (doublons fonctionnels — ex. `post-edit-typecheck`, couvert par `post-tool-batch-typecheck`) : la liste vit dans `EXCLUDED_SLUGS` de [`.claude/sync-hooks.mjs`](.claude/sync-hooks.mjs).
