...
**Garde-fous CI** ([.github/workflows/ci.yml](.github/workflows/ci.yml)) : sur chaque PR, `pnpm typecheck` + `pnpm test` + `pnpm validate:registry` (valide `registry.json` contre [`registry/registry.schema.json`](registry/registry.schema.json) — champ requis manquant, énumération invalide ou champ inconnu = échec) + `node .claude/sync-hooks.mjs --check` (échoue si le registre a dérivé des `.mjs`). Le schéma a `additionalProperties: false` au niveau du hook : tout champ non consommé par le code (ex. anciens `id`/`votes`) est rejeté — il doit rester aligné avec `src/types/hook.ts`. Côté session, deux hooks Stop calqués sur le même patron auto-désactivable surveillent les fichiers modifiés : `stop-per-file-coverage` (couverture ≥80 %) et `stop-per-file-lint` (ESLint).
