...
**Install language-aware** : `install` filtre les hooks par la toolstack détectée du projet courant (`detectToolstack` : `package.json`/`tsconfig.json` → typescript ; `pyproject.toml`/`requirements.txt`/`setup.py`/`setup.cfg`/`Pipfile`/`uv.lock`/`poetry.lock` → python). `filterHooksByStack` garde les hooks universels (sans `stack`) + ceux dont la stack intersecte la détection ; **si la détection ne trouve aucune toolchain connue (Go, Rust, projet sans manifeste), seuls les hooks universels sont installés** — jamais de hooks tsc/ruff/pytest que le projet ne peut pas exécuter — avec un message expliquant comment surcharger ; `--stack=typescript|python|all` surcharge (défaut `auto`). Conséquence : un hook universel qui ne marche que sur Node doit être taggé `stack: ["typescript"]` (ex. `post-write-biome`, `stop-run-tests`, `session-start-node-version-check` — un équivalent python existe dans le registre) et un hook dont le comportement dépend du langage doit être **language-aware** (ex. `stop-quality-check` : tsc/biome côté TS, ruff/pyright via `uv` côté Python ; `task-completed-test-gate` : test npm côté Node, `uv run pytest` côté Python). Sur un install 100 % Python, les hooks avec une **variante Python** (`python_script_path` + `python_code_snippet`) sont installés en `.py` (commande `python3`) avec tests pytest (`doInstallTests` écrit `tests/hooks/test_<slug>.py`) ; les autres retombent en `.mjs` (fallback listé dans le résumé). `update`/`detectScriptChanges` comparent la variante installée. `findInstalledSlugs` reconnaît les deux fingerprints (`//` et `# @hookstack`).
...
7. Lancer `pnpm readme:counts` — régénère les comptes de hooks cités dans les docs à état courant (CLAUDE.md, READMEs, `doc/product`, `okf/business`…) et le bloc `<!-- HOOK_COUNTS -->` du README, depuis le registre (`default_on`/`stack`/variantes `.py`) et `.claude/settings.json` (dogfood). Committer ces artefacts. La CI `--check` échoue sinon.
...
## Timeline d'évolution (/evolution)

Met en valeur la croissance du catalogue « en public » (marketing/communauté). **Source de vérité : l'historique git** — la date de premier ajout (`git log --diff-filter=A`) de chaque `.claude/hooks/*.mjs`. Aucune date saisie à la main.

**Mécanique (générateur déterministe, pas un git pre-commit)** : [`.claude/hooks-timeline.mjs`](.claude/hooks-timeline.mjs) calque le pattern de `sync-hooks.mjs` — fonctions pures + DI, testé dans [`tests/lib/hooks-timeline.test.mjs`](tests/lib/hooks-timeline.test.mjs). `pnpm timeline` régénère **3 artefacts dérivés** :

1. `registry/hooks-timeline.json` — données consommées par le front via [`src/lib/timeline.ts`](src/lib/timeline.ts)
2. `public/hooks-timeline.svg` — heatmap « style contribution GitHub » embarqué dans le README (un SVG committé, car GitHub nettoie le CSS des tableaux markdown)
3. bloc README entre `<!-- HOOKS_TIMELINE:START/END -->` (inséré avant le premier `## ` au premier passage)

`--check` (CI) échoue si un artefact a dérivé · `--dry-run` n'écrit rien. **Ne jamais éditer ces 3 artefacts à la main** : `pnpm timeline` les écrase. Front : page Server Component [`src/app/evolution/page.tsx`](src/app/evolution/page.tsx) + dashboard client [`src/components/EvolutionDashboard.tsx`](src/components/EvolutionDashboard.tsx) (heatmap interactif + courbe cumulée + liste récente). **La rampe de couleur `LEVEL_COLORS` est dupliquée** entre le `.mjs` et `src/lib/timeline.ts` (monochrome, fidèle à `--color-brand: #fff`) — garder les deux identiques.
...
**Hooks Python** : un hook peut avoir une **variante Python** — un fichier `.claude/hooks/<slug>.py` (stdlib uniquement, `#!/usr/bin/env python3`, fingerprint `# @hookstack <slug>` en ligne 2) + un test `tests/hooks/test_<slug>.py` (pytest). Les champs `implementation.python_script_path` / `python_code_snippet` / `python_test_snippet` du registre en dérivent via le sync (miroir du `.py` sur disque, comme `code_snippet` l'est du `.mjs`). Le CLI (`--stack=python` ou projet Python détecté) installe alors la variante `.py` avec une commande `python3 … .py`, et n'installe **jamais** de tests vitest sur un projet Python (pytest uniquement) — le CI du projet reste Python-only. Sans variante, le CLI retombe sur le `.mjs` (fallback affiché). Pattern : fonction pure `run(...)` (signature identique au `.mjs`, deps par kwargs injectables), garde `if __name__ == "__main__":` qui lit stdin JSON, marshalle le résultat et écrit stdout/stderr + exit code selon le contrat. Toujours préférer `uv run <tool>` à l'appel direct (`ruff`, `pytest`, `pyright`). Pour transcrire un hook : copier la logique du `.mjs`, ajouter le `.py` + `python_script_path` + `tests/hooks/test_<slug>.py`, puis `node .claude/sync-hooks.mjs`.

**Parité Python/JS** : pour chaque hook qui a les deux variantes, la **couverture de tests Python doit être ≥ couverture JS**. La porte CI `pnpm check:python-parity` ([scripts/check-python-coverage-parity.mjs](scripts/check-python-coverage-parity.mjs), exécutée à chaque PR) compte `def test_*` dans `tests/hooks/test_<slug>.py` vs `it(...)` dans `tests/hooks/<slug>.test.mjs` et **échoue si Δ < 0**. Cela garantit qu'une régression ne rend pas la variante `.py` (déployée via la CLI sur les projets Python) moins testée que la `.mjs`. Quand on ajoute un `it()` au test `.mjs`, ajouter le `def test_*` Python correspondant (même scénario, `importlib.util` avec les mêmes fakes). Exceptions documentées dans [`SURFACE_EXCEPTIONS`](scripts/check-python-coverage-parity.mjs) — la liste ne doit que **décroître** : si la variante Python d'un hook listed acquiert la même surface que sa `.mjs`, retirer l'exception du script.

**Hooks actifs** : voir `.claude/settings.json` (généré par sync). 93 hooks du catalogue sont actifs sur ce projet, chacun avec un test dans `tests/hooks/`. Pour consulter la liste complète : `node .claude/sync-hooks.mjs --dry-run`. Certains hooks du catalogue sont volontairement **exclus localement** (doublons fonctionnels — ex. `post-edit-typecheck`, couvert par `post-tool-batch-typecheck`) : la liste vit dans `EXCLUDED_SLUGS` de [`.claude/sync-hooks.mjs`](.claude/sync-hooks.mjs).
